...
local function init(args)
		...
		widget_template = {
			{
					...
					{
						{
							...
							forced_width = h / 10,
							id = "status",
							bg = beautiful.dock_unfocused_indicator_bg or "#313244",
							shape = inner_shape,
				...
				id = "bg",
				widget = wibox.container.background,
				bg = beautiful.dock_bg or "#1e1e2e",
				shape = inner_shape,
			},
			widget = wibox.container.margin,
			margins = h / 10,
				...
				local function hover(p, t) --so gc can collect all the timed objects that are flying around
					local on_hover = rubato.timed({
						...
						subscribed = function(pos)
							self:get_children_by_id("bg")[1].bg = chel.col_shift(beautiful.dock_bg or "#181825", pos)
						end,
					})
					on_hover.target = t
				end
			...
			update_callback = function(self, c, _, _) --praying to the gc that this is getting cleared properly, didnt show problems in testing
				...
				local bg_col = beautiful.dock_unfocused_indicator_bg or "#313244"
				local bg_focus_col = beautiful.dock_indicator_bg or "#cba6f7"
	...
	local dock_box = awful.popup({
		...
		widget = {
			...
			widget = wibox.container.background,
			bg = beautiful.dock_bg or "#181825",
			shape = inner_shape,
		},
	})
