...
local stats_text = wibox.widget({
	...
	markup = helpers.ui.colorize_text("Stats", "#6c7086"),
	valign = "center",
...
local function cpu()
	...
	watch(
		...
		function(_, stdout)
			...
			tooltip:set_markup_silently(helpers.ui.colorize_text(cpu_value .. "%", "#89dceb"))
	...
	do
		...
		function(stdout)
			...
			sh -c "cat ]] .. temp_path .. [["
				...
				tooltip:set_markup_silently(helpers.ui.colorize_text(temp_value .. "°C", "#89dceb"))
				collectgarbage("collect")
			end)
		end
	)
...
local function ram()
	...
	watch(
		...
		function(_, stdout)
			...
			tooltip:set_markup_silently(helpers.ui.colorize_text(string.format("%.1f", used / 1000) .. "G", "#89b4fa"))
			collectgarbage("collect")
		end
	)
...
local function hdd()
	...
	watch([[bash -c "df -h /home|grep '^/' | awk '{print $5}'"]], 180, function(_, stdout)
		...
		tooltip:set_markup_silently(helpers.ui.colorize_text(space_consumed .. "%", "#89b4fa"))
		collectgarbage("collect")
	end)
